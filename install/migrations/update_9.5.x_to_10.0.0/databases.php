...
if (!$DB->tableExists('glpi_databasetypes')) {
   $query = "CREATE TABLE `glpi_databasetypes` (
   ...
   $DB->queryOrDie($query, "10.0 add table glpi_databasetypes");
}

if (!$DB->tableExists('glpi_databasecategories')) {
   $query = "CREATE TABLE `glpi_databasecategories` (
         `id` int NOT NULL AUTO_INCREMENT,
         `name` varchar(255) DEFAULT NULL,
         `comment` text,
         `date_mod` timestamp NULL DEFAULT NULL,
   ...
   $DB->queryOrDie($query, "10.0 add table glpi_databasecategoriess");
}
...
if (!$DB->tableExists('glpi_databases')) {
   $query = "CREATE TABLE `glpi_databases` (
         ...
         `databasetypes_id` int NOT NULL DEFAULT '0',
         `databasecategories_id` int NOT NULL DEFAULT '0',
         ...
         KEY `databasetypes_id` (`databasetypes_id`),
         KEY `databasecategories_id` (`databasecategories_id`),
...
if (!$DB->tableExists('glpi_databases_items')) {
   $query = "CREATE TABLE `glpi_databases_items` (
           `id` int NOT NULL AUTO_INCREMENT,
          `databases_id` int NOT NULL DEFAULT '0',
          `items_id` int NOT NULL DEFAULT '0',
          `itemtype` varchar(100) NOT NULL DEFAULT '',
          PRIMARY KEY (`id`),
          UNIQUE KEY `unicity` (`databases_id`,`items_id`,`itemtype`),
          KEY `item` (`itemtype`,`items_id`)
      ) ENGINE = InnoDB ROW_FORMAT = DYNAMIC DEFAULT CHARSET = {$default_charset} COLLATE = {$default_collation};";
   $DB->queryOrDie($query, "10.0 add table glpi_databases_items");
}
