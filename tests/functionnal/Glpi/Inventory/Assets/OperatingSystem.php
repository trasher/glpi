...
class OperatingSystem extends AbstractInventoryAsset {
   ...
   protected function assetProvider() :array {
      return [
         [
            ...
            'asset'  => [
              'standard' => '{"arch": "x86_64", "boot_time": "2018-10-02 08:56:09", "dns_domain": "teclib.infra", "fqdn": "glpixps.teclib.infra", "full_name": "Fedora 28 (Workstation Edition)", "hostid": "a8c07701", "kernel_name": "linux", "kernel_version": "4.18.9-200.fc28.x86_64", "name": "Fedora", "timezone": {"name": "CEST", "offset": "+0200"}, "version": "28 (Workstation Edition)", "operatingsystems_id": "Fedora 28 (Workstation Edition)", "operatingsystemversions_id": "28 (Workstation Edition)", "operatingsystemarchitectures_id": "x86_64", "operatingsystemkernels_id": "linux", "operatingsystemkernelversions_id": "4.18.9-200.fc28.x86_64", "operatingsystemeditions_id": ""}',
               'managed' => '{"arch": "x86_64", "boot_time": "2018-10-02 08:56:09", "dns_domain": "teclib.infra", "fqdn": "glpixps.teclib.infra", "full_name": "Fedora 28 (Workstation Edition)", "hostid": "a8c07701", "kernel_name": "linux", "kernel_version": "4.18.9-200.fc28.x86_64", "name": "Fedora", "timezone": {"name": "CEST", "offset": "+0200"}, "version": "28 (Workstation Edition)", "operatingsystems_id": "Fedora", "operatingsystemversions_id": "28 (Workstation Edition)", "operatingsystemarchitectures_id": "x86_64", "operatingsystemkernels_id": "linux", "operatingsystemkernelversions_id": "4.18.9-200.fc28.x86_64", "operatingsystemeditions_id": ""}'
            ]
         ], [
            'nodes'  => [
               'ARCH'           => '64-bit',
            ...
            'asset'  => [
               'managed' => [
                  ...
                  'operatingsystemversions_id'        => '7',
                  'operatingsystemeditions_id'        => 'Enterprise'
               ],
               'standard' => [
                  'operatingsystemarchitectures_id'   => '64-bit',
                  'operatingsystemkernels_id'         => 'MSWin32',
   ...
   public function testPrepare($nodes, $expected) {
      ...
      $conf = new \mock\Glpi\Inventory\Conf();
      $this->boolean($conf->saveConf(['manage_osname' => 1]))->isTrue();
      $this->boolean((bool)$conf->manage_osname)->isTrue();
      $this->boolean(
         $asset->checkConf($conf)
      )->isTrue();
      $result = $asset->prepare();
      if (!is_array($expected['managed'])) {
         $object = json_decode($expected['managed']);
      } else {
         $object = clone $json->content->operatingsystem;

         foreach ($expected['managed'] as $name => $value) {
            $object->$name = $value;
            /*if (isset($mapping[$name])) {
               $object->{$mapping[$name]} = $value;
            }*/
         }
         $tz = new \stdClass();
         $tz->name = 'CEST';
      ...
      $this->object($result[0])->isEqualTo($object, print_r($result[0], true) . print_r($object, true));

      //not managed osname
      $asset = new \Glpi\Inventory\Asset\OperatingSystem($computer, (array)$json->content->operatingsystem);
      $asset->setExtraData((array)$json->content);
      $this->boolean($conf->saveConf(['manage_osname' => 0]))->isTrue();
      $this->boolean((bool)$conf->manage_osname)->isFalse();
      $this->boolean(
         $asset->checkConf($conf)
      )->isTrue();
      $result = $asset->prepare();
      if (!is_array($expected['standard'])) {
         $object = json_decode($expected['standard']);
      } else {
         $object = clone $json->content->operatingsystem;

         foreach ($expected['standard'] as $name => $value) {
            $object->$name = $value;
            /*if (isset($mapping[$name])) {
               $object->{$mapping[$name]} = $value;
            }*/
...
<REQUEST>
  ...
  <QUERY>INVENTORY</QUERY>
    ...
    * @dataProvider assetProvider
    */
   public function OLDtestPrepare($json, $expected) {
      $this->login();
      $converter = new \Glpi\Inventory\Converter;
      ...
      $result = $asset->prepare();
      $this->object($result[0])->isEqualTo(json_decode($expected['managed']));
      ...
      $result = $asset->prepare();
      $this->object($result[0])->isEqualTo(json_decode($expected['standard']));
   }
   ...
   public function testWrongMainItem() {
      ...
      $asset = new \Glpi\Inventory\Asset\OperatingSystem($mainasset);
      $this->exception(
         function () use ($asset) {
            $asset->prepare();
         }
      )->message->contains('Operating systems are handled for computers only.');
   }
   ...
   public function testHandle() {
      ...
      $asset->setExtraData((array)$json->content);
      $result = $asset->prepare();
      $this->object($result[0])->isEqualTo(json_decode($expected['asset']['standard']));
