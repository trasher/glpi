...
class Inventory extends InventoryTestCase
{
    private function checkComputer1($computers_id)
    {
       ...
       //remote management
        ...
        $mmodel = $DB->request(['FROM' => \MonitorModel::getTable(), 'WHERE' => ['name' => 'DJCP6']])->next();
        $this->array($mmodel);
        $models_id = $mmodel['id'];
        ...
        $this->array($monitor_link)->isEqualTo($expected);
    ...
    public function testImportComputer()
    {
       ...
       //check created agent
        ...
        $this->string($current['name'])->isIdenticalTo('Monitor import (by serial)');
        $this->string($current['method'])->isIdenticalTo(\Glpi\Inventory\Request::INVENT_QUERY);
        ...
        $this->integer(count($iterator))->isIdenticalTo(1);
        $current = $iterator->next();
        $this->string($current['name'])->isIdenticalTo('Printer import (by serial)');
        $this->string($current['method'])->isIdenticalTo(\Glpi\Inventory\Request::INVENT_QUERY);
        ...
        $this->integer(count($iterator))->isIdenticalTo(1);
        $current = $iterator->next();
        $this->string($current['name'])->isIdenticalTo('Computer import (by serial + uuid)');
        $this->string($current['method'])->isIdenticalTo(\Glpi\Inventory\Request::INVENT_QUERY);
    }
    ...
    public function testUpdateComputer()
    {
        ...
        $current = $iterator->next();
        $this->string($current['name'])->isIdenticalTo('Computer import (by serial + uuid)');
        $this->integer($current['items_id'])->isIdenticalTo($agent['items_id']);
        $this->string($current['method'])->isIdenticalTo(\Glpi\Inventory\Request::INVENT_QUERY);
        ...
        $this->integer(count($logs))->isIdenticalTo(4382);
        ...
        $expected_types_count = [
            ...
            \Log::HISTORY_ADD_SUBITEM => 3244,//network port/name, ip address, VMs, Software
        ...
        $this->integer(count($iterator))->isIdenticalTo(1);
        $current = $iterator->next();
        $this->string($current['name'])->isIdenticalTo('Monitor update (by serial)');
        $this->string($current['method'])->isIdenticalTo(\Glpi\Inventory\Request::INVENT_QUERY);
...
Compiled Tue 28-Sep-10 13:44 by prod_rel_team",
    ...
    public function testImportRefusedFromAssetRules()
    {
        ...
        $current = $iterator->next();
        $this->string($current['name'])->isIdenticalTo('Computer import (by serial + uuid)');
        $this->integer($current['items_id'])->isIdenticalTo($gagent->fields['items_id']);
        $this->string($current['method'])->isIdenticalTo(\Glpi\Inventory\Request::INVENT_QUERY);*/
    }
    ...
    public function testImportPhone()
    {
        ...
        $this->array($computer->fields)->isEqualTo($expected);
