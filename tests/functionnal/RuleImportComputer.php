...
class RuleImportComputer extends DbTestCase {
   ...
   protected function enableRule($name) {
      ...
      $DB->update(
         \RuleImportComputerCollection::getTable(),
         ['is_active' => 1],
         [
            'name'      => ['LIKE', "%$name%"],
            'is_active' => 0
   ...
   private function addComputerIPRules() {
      ...
      $this->addRule(
         ...
         [
            ...
            [
               'condition' => \RuleImportComputer::PATTERN_FIND,
               'criteria'  => 'ip',
               'pattern'   => '1',
            ],
            [
               'condition' => \RuleImportComputer::PATTERN_EXISTS,
               'criteria'  => 'ip',
               'pattern'   => '1',
            ],
         ],
         [
            'action_type' => 'assign',
            'field'       => '_fusion',
            'value'       => \RuleImportComputer::RULE_ACTION_LINK_OR_IMPORT,
         ],
   ...
   protected function refuseProvider() {
      return [
         [    // IP
            ...
            'data'   => ['mac' => 'd4:81:d7:7b:6c:21', 'ip' => '192.168.0.10'],
            'rname'  => 'Global constraint (name)'
         ], [ // IP+name
            'data'   => ['name' => 'pc-01', 'ip' => '192.168.0.10'],
            'rname'  => 'Global import denied'
         ], [ // IP+mac+name
            'data'   => ['name' => 'pc-01', 'mac' => 'd4:81:d7:7b:6c:21', 'ip' => '192.168.0.10'],
            'rname'  => 'Global import denied'
         ]
      ];
   }
   ...
   public function testCreateMacIfnumber() {
      ...
      $this->integer($this->items_id)->isIdenticalTo(0);
      $this->string($this->itemtype)->isIdenticalTo('InventoryUnmanaged'); //not handled yet...
   ...
   protected function ruleactionProvider() {
      $known = \RuleImportComputer::getRuleActionValues();
   ...
   protected function moreCritProvider() {
      return [
         [
            ...
            'expected'  => [
               \RuleImportComputer::PATTERN_ENTITY_RESTRICT => 'Yes'
            ]
         ], [
            'criterion' => 'link_criteria_port',
            'expected'  => [
               \RuleImportComputer::PATTERN_NETWORK_PORT_RESTRICT => 'Yes'
            ]
         ], [
            'criterion' => 'only_these_criteria',
            'expected'  => [
               \RuleImportComputer::PATTERN_ONLY_CRITERIA_RULE => 'Yes'
            ]
         ], [
            'criterion' => 'any_other',
            'expected'  => [
               \RuleImportComputer::PATTERN_FIND => 'is already present',
               \RuleImportComputer::PATTERN_IS_EMPTY => 'is empty'
            ]
         ]
      ];
   }
