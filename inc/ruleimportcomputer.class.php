...
class RuleImportComputer extends Rule {
   ...
   function findWithGlobalCriteria($input) {
      ...
      foreach ($itemtypeselected as $itemtype) {
         ...
         $it_criteria = [
            ...
            'WHERE'  => [], //to fill
            'ORDER'  => "$itemtable.is_deleted ASC"
         ];
   ...
   **/
    ...
    * @return boolean
    */
   public static function initRules($reset = true, $with_plugins = true) {
      global $PLUGIN_HOOKS;

      //FIXME: will reset plugins additions aswell...
      if ($reset) {
         $grule = new Rule();
         $a_rules = $grule->find(['sub_type' => 'RuleImportComputer']);
