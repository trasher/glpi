...
class Database_Item extends CommonDBRelation {
   use Glpi\Features\Clonable;

   static public $itemtype_1 = 'Database';
   static public $items_id_1 = 'databases_id';
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
      ...
      $nb = 0;
      if ($item->getType() == Database::class) {
         if ($_SESSION['glpishow_count_on_tabs'] && !$item->isNewItem()) {
            $nb = self::countForMainItem($item);
         }
         return self::createTabEntry(self::getTypeName(Session::getPluralNumber()), $nb);

      } else if (in_array($item->getType(), Database::getTypes(true))) {
         if ($_SESSION['glpishow_count_on_tabs']) {
            $nb = self::countForItem($item);
         }
         return self::createTabEntry(Database::getTypeName(Session::getPluralNumber()), $nb);
      }
   }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0) {
      ...
      switch ($item->getType()) {
         case Database::class:
            ...
            break;
         default :
            if (in_array($item->getType(), Database::getTypes())) {
    ...
    *
    * @param Database $database  Database object wanted
    *
    * @return void|boolean (display) Returns false if there is a rights error.
   **/
   static function showItems(Database $database) {
      global $DB;
      ...
      if ($database->canAddItem('itemtype')) {
         ...
         Dropdown::showSelectItemFromItemtypes(
               ['items_id_name'   => 'items_id',
                'itemtypes'       => Database::getTypes(true),
         ...
         echo Html::hidden('databases_id', ['value' => $ID]);
   ...
   **/
      ...
      if ($number > 0) {
         ...
         foreach ($databases as $data) {
            ...
            foreach ($instances as $instance) {
               ...
               echo "<a href='" . Database::getFormURLWithID($cID) . "'>" . $name . "</a>";
               echo "</td>";
   ...
   static function getRelationMassiveActionsSpecificities() {
      ...
      $specificities['itemtypes'] = Database::getTypes();
