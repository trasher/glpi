...
class NotificationEventMailing extends NotificationEventAbstract implements NotificationEventInterface {
   ...
   static public function send(array $data) {
      ...
      foreach ($data as $row) {
         ...
         self::handleEmailFiles($mmail, $current);

         //Finally, set mail contents (may have been altered from attachements handling)
   ...
   static protected function handleEmailFiles(GLPIMailer $mmail, QueuedNotification $current) {
      global $DB, $CFG_GLPI;
      ...
      $is_html = !empty($current->fields['body_html']);
      $field = $is_html ? 'body_html' : 'body_text';
      ...
      if (count($document_items)) {
         foreach ($document_items as $doc_i_data) {
            ...
            if (preg_match_all('/'.Document::getImageTag($doc->fields['tag']).'/',
                                 $current->fields[$field], $matches, PREG_PATTERN_ORDER)) {
               ...
               if ($is_html) {
                  ...
                  // Add as attachment, according to configuration
                  $mmail->addAttachment(
                     $image_path,
                     $doc->fields['filename']
                  );
      ...
      foreach ($inline_docs as $docID => $tag) {
         $current->fields[$field] = preg_replace([
