   ...
   public function prepare() :array {
      ...
      foreach ($this->data as $k => &$val) {
         ...
         if (property_exists($vm_val, 'networks') && is_array($vm_val->networks)) {
            foreach ($vm_val->networks as $net_key => $net_val) {
   ...
   public function handle() {
      ...
      if (count($db_vms) == 0) {
         foreach ($value as $val) {
            ...
            $input['is_dynamic']  = 1;
            $computerVirtualmachine->add($input, [], $this->withHistory());
         }
      } else {
         foreach ($db_vms as $keydb => $arraydb) {
            foreach ($value as $key => $val) {
               ...
               if ($sinput == $arraydb) {
                  ...
                  $computerVirtualmachine->update($input, $this->withHistory());
                  unset($value[$key]);
                  unset($db_vms[$keydb]);
                  break;
               }
      ...
      foreach ($this->data as $vm) {
         ...
         if (property_exists($vm, 'uuid') && $vm->uuid != '') {
            ...
            if ($computers_vm_id == 0) {
               ...
               $computers_vm_id = $computervm->add((array)$vm, [], $this->withHistory());
            } else {
               // Update computer
               $computervm->getFromDB($computers_vm_id);
               $input = (array)$vm;
               $input['id'] = $computers_vm_id;
               $computervm->update($input, $this->withHistory());
            }
            //load if new, relaod if not.
            $computervm->getFromDB($computers_vm_id);
            // Manage networks
