...
class Computer extends InventoryAsset
{
   ...
   public function handle() {
      ...
      if (isset($data['_no_rule_matches']) AND ($data['_no_rule_matches'] == '1')) {
         ...
         //FIXME: create and populate ignore equipment
         $this->errors[] = __('No rules match for this equipment.');
      }
   }
   ...
   public function rulepassed($items_id, $itemtype, $rules_id, $ports_id = 0) {
      ...
      if ($items_id == 0) {
         ...
         if ($this->agent->fields['items_id'] != $items_id) {
            $this->agent->update(['id' => $this->agent->fields['id'], 'items_id' => $items_id]);
         }
      }
      ...
      $this->data = [$val];
      $this->handleLinks();
      //and then update $val with possible new/updated data
      $val = $this->data[0];
      ...
      //FIXME: maybe is there a better way... This would override current SESSION values.
      $_SESSION['glpiactiveentities']        = [$entities_id];
      $_SESSION['glpiactiveentities_string'] = $entities_id;
      $_SESSION['glpiactive_entity']         = $entities_id;
      ...
      */
      ...
      $this->item->update(\Toolbox::addslashes_deep((array)$val), $this->with_history);
