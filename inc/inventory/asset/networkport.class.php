...
trait NetworkPort {
   ...
   private function cleanUnmanageds() {
      ...
      $networkport = new \NetworkPort();
      //FIXME: object does not exists yet
      //$unmanaged = new \InventoryUnmanaged();

      $criteria = [
         'FROM'   => \NetworkPort::getTable(),
         'WHERE'  => [
            'itemtype'  => 'InventoryUnmanaged',
            'mac'       => new \QueryParam()
         ]
      ];
      ...
      foreach ($this->ports as $key => $port) {
         ...
         /*foreach (['name', 'mac', 'instantiation_type'] as $field) {
            if (property_exists($port, $field)) {
               $this->networks[$key][$field] = $port->$field;
            }
         }*/
      }
   }
   ...
   private function addNetworkPort(\stdClass $port) {
      ...
      $input = \Toolbox::addslashes_deep((array)$port);
      $input = array_merge(
         $input, [
            'entities_id'  => $this->entities_id,
            'items_id'     => $this->items_id,
    ...
    *
    * @param integer $items_id NetworkPort id
    *
    * @return integer
    */
   private function addNetworkName($items_id) {
      $networkname = new \NetworkName();
      $input = [
         'entities_id'  => $this->entities_id,
         'items_id'     => $this->items_id,
   ...
   private function handleUpdates() {
      ...
      foreach ($this->ports as $key => $data) {
         foreach ($db_ports as $keydb => $datadb) {
            ...
            foreach (['name', 'mac', 'instantiation_type'] as $field) {
               if (property_exists($port, $field)) {
                  $comp_data[$field] = strtolower($port->$field);
               }
            }
            ...
            if ($data->logical_number != $db_lnumber) {
               $networkport->update(
                  [
                     'id'              => $keydb,
      ...
      if (count($db_ports)) {
         foreach ($db_ports as $netpid => $data) {
   ...
   private function handleInstantiation($type, $data, $ports_id, $load) {
      ...
      if ($load === true) {
         $instance->getFromDBByCrit(['networkports_id' => $ports_id]);
   ...
   private function handleCreates() {
      foreach ($this->ports as $port) {
         ...
         $netnames_id = $this->addNetworkName($netports_id);
