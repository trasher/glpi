   ...
   static function showForRoom(DCRoom $room) {
      ...
      foreach ($racks as $row) {
         ...
         if (preg_match('/(\d+),\s?(\d+)/', $row['position'], $position)) {
            $x = $position[1];
            $y = $position[2];
            if (isset($data[$x]) && isset($data[$y]["f$x"])) {
               $in = true;
               $style = '';
               ...
               $data[$y]["f$x"] = "<div data-id='{$rack->getID()}' class='redips-drag'$style>" . $rack->getName() .
      ...
      if (count($outbound)) {
         ...
         foreach ($outbound as $out) {
            ...
            echo "<td><div data-id='{$rack->getID()}' class='redips-drag'$style>" . $rack->getName() .
               "<a href='{$rack->getLinkURL()}'><i class='fa fa-link'></i></a>".
               "<span class='tipcontent'><strong>".__('Name:')."</strong> {$rack->getName()}<br/>
      ...
      for ($i = 0; $i < (int)$room->fields['vis_cols'] + 1; ++$i) {
         if ($i === 0) {
            echo "<th class='redips-trash' title='".__('Drop here to dissociate')."'><span><i class='fa fa-trash-o fa-2x'></i></span></th>";
         } else {
            echo "<th class='redips-mark'>$i</th>";
         }
      }
      echo "</tr></thead>";
      foreach ($data as $row) {
         ...
         foreach ($row as $cell) {
            if (is_int($cell)) {
               echo "<th class='redips-mark'>$cell</th>";
            } else {
               echo "<td>$cell</td>";
            }
         }
      ...
      $js = "$(function(){
         ...
         rd.event.dropped = function (cell) {
            var _div = $(cell).find('div');
            var _content = _div.html();
            var _id = _div.data('id');
            var pos = rd.getPosition();
            $.ajax({
               url: '{$CFG_GLPI['root_doc']}/ajax/moverack.php',
               method: 'POST',
               data: {
                  'id': _id,
                  'pos': {
               ...
               error: function() {
                  alert('".__s('Rack has not been moved')."');
                  rd.moveObject({
                     obj: _div.get(0),
         ...
         /*rd.trash.question = '". __s('Are you sure you want to dissociate this rack?')."';
         ...
         $('#viewgraph td').on('click', function(){
            ...
            if (_this.find('div').length == 0) {
               _this.append('<div class=\'redips-drag\' id=\'new_rack\'>".__("New rack")."</div>');
               rd.enableDrag(true, '#new_rack');
               var pos = rd.getPosition('new_rack');
               window.location = '{$rack->getFormURL()}?room={$room->getID()}&position=' + pos[2] + ','+pos[1];
            }
         });
